...
- **Control Adicional (Actualizado):** Se ha habilitado de forma estricta el Row Level Security (RLS) en Supabase para las tablas `clientes`, `propiedades` y `articulos`. Esto garantiza que la exposición de la llave pública `SUPABASE_ANON_KEY` no pueda ser utilizada para la extracción (Data Scraping) o alteración masiva de datos por usuarios no autenticados, blindando el ecosistema.
 
### 5. Simetría de Contenedores Transversales & Tipografía
- **Regla:** Toda sección principal del sitio web (portafolios, contacto, blog) debe encapsularse en contenedores transversales con ancho máximo maestro de **`1300px`** (`.featured-container`, `.contact-container`), y sus títulos principales deben implementar la clase `.section-title` con la escala dinámica unificada `clamp(32px, 5vw, 48px)` y `font-weight: 900`.
- **Motivo:** Evita rupturas asimétricas o compresiones indebidas de texto entre secciones en pantallas panorámicas, asegurando una experiencia visual balanceada y premium en todo el ecosistema.
